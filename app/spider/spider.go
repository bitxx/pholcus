...
type (
	...
	Spider struct {
		...
		ContinueSpiderWithFailure bool                                                       // 如果启动监测到历史记录中有爬取失败的记录时，true:任务和历史错误同时爬取，false：只爬取历史错误记录
...
func (self *Spider) Start() {
	...
	if !self.reqMatrix.HasFaliure() || (self.reqMatrix.HasFaliure()) && self.ContinueSpiderWithFailure {
